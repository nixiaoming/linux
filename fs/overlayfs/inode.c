...
static void ovl_fill_inode(struct inode *inode, umode_t mode, dev_t rdev,
			   unsigned long ino, int idx)
{
	int xinobits = (BITS_PER_LONG == 64) ? ovl_xino_bits(inode->i_sb) : 0;
	...
	if (inode->i_sb->s_export_op &&
		...
		inode->i_ino = ino;
		if (xinobits && idx)
			inode->i_ino |= (unsigned long)idx << (64 - xinobits);
	} else {
			    ...
			    unsigned int numlower)
{
	...
	int idx = bylower ? lowerpath->layer->idx : 0;
	bool is_dir;
	unsigned long ino = 0;
	...
	ovl_fill_inode(inode, realinode->i_mode, realinode->i_rdev, ino, idx);
