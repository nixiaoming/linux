...
static void __fsnotify_recalc_mask(struct fsnotify_mark_connector *conn)
{
	...
	if (conn->type == FSNOTIFY_OBJ_TYPE_INODE)
		fsnotify_obj_inode(conn->obj)->i_fsnotify_mask = new_mask;
	else if (conn->type == FSNOTIFY_OBJ_TYPE_VFSMOUNT)
		fsnotify_obj_mount(conn->obj)->mnt_fsnotify_mask = new_mask;
}
...
static int fsnotify_attach_connector_to_object(fsnotify_obj_t *obj,
					       unsigned int type)
{
	struct inode *inode = NULL;
	struct fsnotify_mark_connector *conn;
	...
	/*
	 * cmpxchg() provides the barrier so that readers of connector can see
	 * only initialized structure
	 */
	if (cmpxchg(obj, NULL, conn)) {
...
static int fsnotify_add_mark_list(struct fsnotify_mark *mark,
				  fsnotify_obj_t *obj, unsigned int type,
				  int allow_dups)
{
...
int fsnotify_add_mark(struct fsnotify_mark *mark, fsnotify_obj_t *obj,
 ...
 */
struct fsnotify_mark *fsnotify_find_mark(fsnotify_obj_t *obj,
					 struct fsnotify_group *group)
{
	struct fsnotify_mark_connector *conn;
	struct fsnotify_mark *mark;
...
void fsnotify_destroy_marks(fsnotify_obj_t *obj)
{
	struct fsnotify_mark_connector *conn;
